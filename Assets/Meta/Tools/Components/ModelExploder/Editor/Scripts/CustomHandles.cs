...
namespace Meta.Tools.Editor
{
    public class MyHandles
    {
        ...
        public static Vector3 DragHandle(Vector3 position, float handleSize, Handles.DrawCapFunction capFunc, Color colorSelected, out DragHandleResult result, out bool alted, out bool ctrled)
        {
            ...
            switch (Event.current.GetTypeForControl(id))
            {
                case EventType.MouseDown:
                    if (HandleUtility.nearestControl == id && (Event.current.button == 0 || Event.current.button == 1))
                    {
                        ...
                        alted = Event.current.alt;
                        ctrled = Event.current.control;
                    }
                    break;
                ...
                case EventType.ScrollWheel:
                    if (GUIUtility.hotControl == id)
                    {
                        result = DragHandleResult.ScrollWheel;
